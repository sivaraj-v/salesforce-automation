...
io.on('connection', socket => {
  ...
  global.scratch_org_alias = "";const config = {
  ...
  app.use(function(req, res, next) {
    res.setHeader('Access-Control-Allow-Origin', 'http://127.0.0.1:8080');res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS, PUT, PATCH, DELETE');res.setHeader('Access-Control-Allow-Headers', 'X-Requested-With,content-type');res.setHeader('Access-Control-Allow-Credentials', !0);next()
    ...
    function processInstallation_emit(i) {
      if (obj_temp.count_LifeCycle_install_pkg > i) {
        ...
        console.log(val_frm_pkg_json);Promise.coroutine(function*() {
          var command = '';
          ...
          if (response.success) {
            ...
            var checkInstallationStatus = setInterval(function() {
              spinner.stop();spinner.start('Package install requested..');
              socket_message = "Installing package number" + i;console.log(installationStatusCheckCommand);Promise.coroutine(function*() {
                spinner.stop();spinner.start('Loading...');
                ...
                if (response.success) {
                  ...
                  if (installationSuccessResolvedArray && installationSuccessResolvedArray.length > 0) {
                    spinner.stop();spinner.succeed(success(installationSuccessResolvedArray[0]));
                    socket_message = installationSuccessResolvedArray[0];for_increment_install(i);clearInterval(checkInstallationStatus)
                  }
                  spinner.stop();spinner.start('Installation process in queue...');
                  socket_message = "Installation process in queue..."
                } else {
                  console.log(error('Package queue failed, Please contact administrator'));
                  socket_message = "Package queue failed, Please contact administrator";console.log(response)
                }
              })()
            }, 120000)
          } else {
            console.log(error('Package installation got failed'));
            socket_message = "Package installation got failed";spinner.stop()
          }
        })()
      } else {
        console.log(success('Package installation process completed'));
        socket_message = "Package installation process completed";socket.emit('so_creation', socket_message);setInterval(function() {
    ...
    function processInstallation_init(value) {
      ...
      if (obj_temp.count_LifeCycle_install_pkg > 0) processInstallation_emit(0);else
    ...
    function responseEmit(params) {
      ...
      socket_SO_userdata = JSON.parse(params);socket.emit('so_creation', socket_message);socket.emit('so_creation_org_details', socket_SO_userdata);res.send(req.body);fs.readFile('config/configdata.json', 'utf8', function readFileCallback(err, data) {
        processInstallation_init(data)
      })
    }
    function displayScratchOrg(value) {
      socket_message = "🆕 Displaying SO Details 🌟"
      spinner.start('Loading..');Promise.coroutine(function*() {
        const response = yield cmd.run('sfdx force:org:display -u SivaSO3 --json');
        if (response.success) {
          socket_message = "😀 Copy your SO Details 🌟";responseEmit(response.message)
          spinner.stop()
        } else {
          socket_message = "👺 Someting went wrong contact admin 👀";console.log(error('Invalid Comment, Please contact administrator'));spinner.stop()
        }
      })()
    }
    function generatePassword(value) {
      socket_message = "🏋️‍ Genrating password... 📢";spinner.start('Loading..');setTimeout(() => {
        spinner.color = 'yellow';
        spinner.text = 'Generating password...';
        socket_message = "🏋️‍ Please wait... Generating password 📢"
      }, 1000);Promise.coroutine(function*() {
        const response = yield cmd.run('npm -v');
        if (response.success) {
          spinner.stop();spinner.succeed('Password generated successfully');
          socket_message = "Password generated successfully ✔️";displayScratchOrg(value)
        } else {
          socket_message = "👺 Someting went wrong contact admin 👀";console.log(error('Invalid Comment, Please contact administrator'));spinner.stop()
        }
      })()
    ...
    function create_scratch_org(value, process) {
      socket_message = "👨🏻‍💻 Creating SO for " + scratch_org_alias;spinner.start('Loading..');setTimeout(() => {
        spinner.color = 'yellow';
        spinner.text = 'Creating scratch org...';
        socket_message = "Please wait... 🕑 creating SO for " + scratch_org_alias
      }, 1000);
      scratch_org_alias = value;Promise.coroutine(function*() {
        const response = yield cmd.run('npm -v');
        if (response.success) {
          spinner.succeed('Org created successfully');
          socket_message = "SO successfully ⛳️ created for " + scratch_org_alias;generatePassword(scratch_org_alias)
        } else {
          socket_message = "👺 Someting went wrong contact admin 👀";console.log(error('Invalid Comment, Please contact administrator'));spinner.stop()
        }
      })()
    ...
    function create_Project(value, process) {
      socket_message = "📁 Creating project...🕔 ";spinner.start('Loading..');setTimeout(() => {
        spinner.color = 'yellow';
        ...
        if (response.success) {
          spinner.succeed('Project created successfully');fs.exists('./config/project-scratch-def.json', function(exists) {
            if (exists) {
              fs.readFile('./config/project-scratch-def.json', 'utf8', function readFileCallback(err, data) {
                fs.writeFile('./' + value + '/config/project-scratch-def.json', data, 'utf8', function readFileCallback(err, data) {
                  ...
                  socket_message = " 📩 Project Successfully Created ⛳️ ";create_scratch_org(scratch_org_alias)
                })
              })
            }
          })
    ...
    function cmd_Exec(value, process) {
      spinner.start('Loading..');setTimeout(() => {
        spinner.color = 'yellow';
        spinner.text = process
      }, 1000);Promise.coroutine(function*() {
        const response = yield cmd.run(value);
        if (response.success) {
          spinner.stop();console.log(success('User LoggedIn successfully'));console.log(obj_temp.projectName);create_Project(obj_temp.projectName);
